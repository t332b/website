...
## 1.1 primary / non-primary と埋め込みの設計

- **primary release**: ジャケ画像を表示し、埋め込みプレーヤーは出さない。リンクのみ表示。
- **non-primary release**（よその作品への参加）: ジャケ画像は著作権配慮で出さない。代わりに埋め込みプレーヤーをジャケ大で表示。優先順は Spotify → Apple → Bandcamp → YouTube（最初に有効な embed を 1 件だけ）。
- **リンク一覧**: 全リリースで、`data.links` にある全リンクをサービスアイコン＋ラベルで表示。`other` は地球儀アイコン＋「外部サイト」。
- **Bandcamp**: 埋め込みは **数値の album/track ID** が必須。slug 形式の URL の場合は embed を生成せず、リンクのみ表示。
- **works index とモーダル**: 上記ルールを同じにしている。index の non-primary カードの埋め込みサイズ・表示バランスは既存のまま（いじらない）。
